begin;
alter table public.masses add column if not exists category text check (category in ('mass','devotion','other'));
create or replace function public.admin_add_event(p_token uuid, p_start_time timestamptz, p_title text, p_suggested_spots integer, p_category text)
...
begin
  ...
  insert into public.masses(start_time,title,suggested_spots,is_extra,category) values (p_start_time,btrim(p_title),p_suggested_spots,p_category='devotion',p_category);
end $$;
revoke all on function public.admin_add_event(uuid,timestamptz,text,integer,text) from public,anon,authenticated;
grant execute on function public.admin_add_event(uuid,timestamptz,text,integer,text) to anon;
...
begin
  ...
  while curr_date <= p_end_date loop
    if extract(dow from curr_date)::integer = any(p_days) and (
      ...
      insert into public.masses (start_time, title, suggested_spots, is_extra, series_id, category)
      values (curr_ts, btrim(p_title), p_suggested_spots, p_category = 'devotion', v_series_id, p_category);
...
revoke all on function public.admin_add_pattern_events(uuid,text,integer,boolean,integer[],time,date,date,text,integer,integer,integer[],text) from public,anon,authenticated;
grant execute on function public.admin_add_pattern_events(uuid,text,integer,boolean,integer[],time,date,date,text,integer,integer,integer[],text) to anon;
revoke all on function public.admin_update_event(uuid,uuid,text,text,time,integer,boolean,text) from public,anon,authenticated;
grant execute on function public.admin_update_event(uuid,uuid,text,text,time,integer,boolean,text) to anon;
commit;
