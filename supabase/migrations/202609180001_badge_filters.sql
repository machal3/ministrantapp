-- Warunki odznak: które służby wliczają się do celu (krok 2 kreatora).
-- Apply after 202609170001_badge_definitions.sql. Safe to rerun: keeps existing definitions.
begin;

alter table public.badge_definitions
  add column if not exists filters jsonb not null default '{}';

-- Stara 7-argumentowa wersja jest zastępowana 8-argumentową (przeciążenie nie wchodzi w grę).
drop function if exists public.admin_upsert_badge(uuid, uuid, text, text, text, integer, integer);
...
begin
  ...
  -- Nieznane klucze są odrzucane, aby starsze i nowsze wersje aplikacji były zgodne.
  select jsonb_object_agg(key, value) into f from jsonb_each(f)
    where key in ('weekdays', 'timeFrom', 'timeTo', 'dates', 'title', 'celebrant', 'occasion', 'category', 'dayMark', 'dayMarkText', 'perDay');
  f := coalesce(f, '{}');
