...
interface Props {
  ...
  onPrefetch?: (week: string) => void;
}

export default function WeekNavigator({ week, onChange, onPrefetch }: Props) {
  ...
  return <div className="week-navigator">
    ...
    <nav className="week-controls" aria-label="Wybór tygodnia">
      <button
        ...
        onMouseEnter={() => onPrefetch?.(shiftDate(week, -7))}
        onTouchStart={() => onPrefetch?.(shiftDate(week, -7))}
      >
        <ChevronLeft size={20} />
      </button>
      <button
        className="today-button"
        onClick={() => onChange(weekStart())}
        onMouseEnter={() => onPrefetch?.(weekStart())}
        onTouchStart={() => onPrefetch?.(weekStart())}
      >
        Dziś
      </button>
      <button
