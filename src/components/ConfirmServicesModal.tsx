...
import { CalendarDays, Check, HeartHandshake, LoaderCircle, RefreshCw, UserRound, X } from 'lucide-react';
import { dateKey, polishDate, timeSlot } from '../lib/dates';
import type { Mass } from '../types/database';
import Modal from './Modal';

interface Props { mass: Mass; name: string; remaining: number; completed: number; busy: boolean; error: string; onAnswer: (attended: boolean) => void; onRetry: () => void; onChangePerson: () => void }
export default function ConfirmServicesModal({ mass, name, remaining, completed, busy, error, onAnswer, onRetry, onChangePerson }: Props) {
  const focus = useRef<HTMLDivElement>(null);
  useEffect(() => { focus.current?.focus(); }, [mass.id]);
  return <Modal title="Potwierdź swoją obecność" onClose={() => {}} dismissible={false} busy={busy} className="confirmation-queue-modal" initialFocusRef={focus}>
    ...
    <button className="button secondary confirmation-change-person" disabled={busy} onClick={onChangePerson}><UserRound size={16} />To nie ja — zmień osobę</button>
  </Modal>;
}
