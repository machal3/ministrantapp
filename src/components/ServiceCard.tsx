...
export default function ServiceCard({ service, activeId, featured, now, busy, onAction, onOpenDay }: Props) {
  ...
  return <article className={`service-card ${featured ? 'next-service' : ''} ${excused ? 'service-excused' : ''}`} aria-label={`${mass.title}, ${day}, ${timeSlot(mass.start_time).slice(0, 5)}`}>
    {featured && <h2>Najbliższa służba</h2>}
    <div className="service-date">
      <time className="service-time" dateTime={mass.start_time}>{timeSlot(mass.start_time).slice(0, 5)}</time>
      <div className="service-calendar"><strong>{polishDate(day, { weekday: 'long' })}</strong><span>{polishDate(day, { day: 'numeric', month: 'long', year: 'numeric' })}</span>{relative && <span className="today-badge">{relative}</span>}</div>
    ...
    <div className="service-main">
      <div className="service-description"><div className="service-title"><h3>{mass.title}</h3>{mass.title.trim().toLocaleLowerCase('pl') !== categoryLabel.toLocaleLowerCase('pl') && <span className="service-category">{categoryLabel}</span>}</div>
    ...
    <div className="service-companions">
      ...
      {companions.length ? <ul aria-label={excused ? 'Zadeklarowani na ten termin' : 'Służą z Tobą'}>{companions.map(person => <li key={person.server_id}>{person.name}</li>)}</ul>
