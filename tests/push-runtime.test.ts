...
it('dispatch requires cron authorization, expires invalid endpoints and acknowledges successful sends',async()=>{
 ...
 runInNewContext(js,{console:{error:log},Deno:{env:{get:()=> 'secret'},serve:(fn:typeof handler)=>{handler=fn;}},createClient:()=>({rpc}),webpush:{setVapidDetails:vi.fn(),sendNotification:send},Response,URL,Date,Intl});
 ...
 expect(log).toHaveBeenCalledWith('push_delivery_failures',expect.objectContaining({failureStatuses:{unsupported_endpoint:1}}));
 expect(JSON.stringify(log.mock.calls)).not.toContain('https://');
 rpc.mockResolvedValue({data:null,error:{code:'PGRST202',message:'private details'}});
 const failure=await handler(new Request('https://local',{method:'POST',headers:{'x-cron-secret':'secret'}}));
 expect(failure.status).toBe(500);
 expect(await failure.json()).toEqual({error:'Notification dispatch failed',stage:'claim_push_notifications'});
 expect(log).toHaveBeenCalledWith('push_dispatch_failed',{stage:'claim_push_notifications',code:'PGRST202'});
